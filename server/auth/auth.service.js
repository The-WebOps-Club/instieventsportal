...
function hasAdminRole(roleRequired) {
  if (!roleRequired) throw new Error('Required role needs to be set');

  return compose()
    .use(isAdmin())
    .use(function meetsRequirements(req, res, next) {
      if (config.adminRoles.indexOf(req.user.role.name) >= config.adminRoles.indexOf(roleRequired)) {
        next();
      }
      else {
        res.send(403);
...
function signToken(id) {
  return jwt.sign({ _id: id }, config.secrets.session, { expiresInMinutes: 60*24*365*5 });
}
